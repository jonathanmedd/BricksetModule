Describe "Module Manifest ->" {
    ...
    It "has a valid module manifest" {
        {Test-ModuleManifest -Path $ENV:BHProjectPath\Release\Brickset\Brickset.psd1} | Should -Not -Throw
    }
...
Describe "Function Help -> " {
    ...
    $ModulePath = "$ENV:BHProjectPath\Release\Brickset\Brickset.psd1"
    Import-Module $ModulePath -Force

    $Functions = @(Get-Command -Module Brickset -CommandType Function | ForEach-Object { @{Name = $_.Name } })
    ...
    AfterAll {
        Remove-Module -Name Brickset -Force
    }
}